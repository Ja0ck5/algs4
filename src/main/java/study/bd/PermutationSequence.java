...
public class PermutationSequence {
    ...
    public String getPermutation(int n, int k) {
        ...
        dfs(0, path,fact, used);
        return path.toString();
    }

    private void dfs(int index, StringBuilder path, int[] fact, boolean[] used){
        if(index == n){
            return;
        }

        int cnt = fact[n - 1 - index];
        for(int i =1 ;i <= n; i++){
            if(used[i]){
                continue;
            }
            ...
            path.append(i);
            used[i] = true;
            dfs(index + 1, path, fact, used);
            return;
        }

    }

    private void calFact(int[] fact){
        fact[0] = 1;
        for(int i=1; i <=n; i++){
            fact[i] = fact[i - 1] * i;
        }
